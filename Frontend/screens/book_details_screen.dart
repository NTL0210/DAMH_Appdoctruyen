...
class _BookDetailsScreenState extends State<BookDetailsScreen>
  ...
  Future<void> _handleFollowToggle() async {
    ...
    if (accountId == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(_t('account_not_found')),
    ...
    if (comicId == null || comicId.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(_t('comic_id_not_found')),
            backgroundColor: Colors.red,
            duration: Duration(seconds: 2),
          ),
        );
    ...
    try {
      ...
      if (mounted) {
        if (result['success'] == true) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(result['message'] ?? (_isFollowing ? _t('followed_success') : _t('unfollowed_success'))),
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(result['error'] ?? _t('generic_error')),
              backgroundColor: Colors.red,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${_t('error_prefix')} $e'),
            backgroundColor: Colors.red,
            duration: const Duration(seconds: 3),
          ),
        );
    ...
    try {
      ...
      if (widget.slug != null) {
        ...
        if (matchingComic == null) {
          ...
          setState(() {
            _loading = false;
            _error = _t('comic_not_found_long');
          });
          return;
        }
    ...
    return SliverAppBar(
      ...
      actions: [
        ...
        IconButton(
          ...
          tooltip: _isFollowing ? _t('unfollow') : _t('follow'),
        ),
        IconButton(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
                  '${_t('author')}: ${author.isNotEmpty ? author.join(', ') : '—'}',
            style: TextStyle(
              fontSize: 16,
              color: Theme.of(context).textTheme.bodyMedium?.color,
            ),
          ...
          ElevatedButton.icon(
            ...
            icon: Icon(
              ...
              size: 20,
                  ...
                  label: Text(
                    _isFollowing ? _t('unfollow') : _t('follow'),
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Text(
                        widget.slug != null
                            ? '${_t('chapters')}: ${_chapterCount()}'
                            : '${_t('pages')}: ${widget.book?.pages ?? '-'}',
                  style: TextStyle(
                    fontSize: 14,
  ...
  Widget _buildDescriptionCard(String contentHtml) {
    final String description =
        _stripHtml(contentHtml).isNotEmpty ? _stripHtml(contentHtml) : _t('no_description');
    ...
    return Column(
      ...
      children: [
        ...
        _buildInfoCard(
          ...
          title: _t('author'),
          value: author.isNotEmpty ? author.join(', ') : _t('not_updated'),
        ),
        const SizedBox(height: 12),
        _buildInfoCard(
          icon: Icons.category,
        ...
        _buildInfoCard(
          ...
          title: _t('chapter_count'),
          value: '$chapterCount ${_t('chapters')}',
        ),
        const SizedBox(height: 12),
        _buildInfoCard(
          icon: Icons.update,
  ...
  Widget _buildStoryInfo() {
    ...
    return Column(
      children: [
        _buildInfoRow(_t('author'), author.isNotEmpty ? author.join(', ') : _t('not_updated')),
        _buildInfoRow(_t('category'), category.isNotEmpty ? ((category.first as Map<String, dynamic>)['name'] ?? '').toString() : 'Comic'),
        _buildInfoRow(_t('chapter_count'), '$chapterCount ${_t('chapters')}'),
        _buildInfoRow(_t('status'), _t('updating')),
      ],
    );
  }
        ...
        child: SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: Text(
              widget.slug != null
                    ? (chapters.isNotEmpty
                      ? '${_t('read_from_chapter')} ${chapters.first['chapterName'] ?? chapters.first['chapter_name']}'
                      : _t('no_chapters'))
